...
import { any, extend, makeArray, setTimeoutOnce } from "zeta-dom/util";
...
function initMenuKeystroke(container, options) {
    ...
    function handleEvent(e) {
        ...
        const prev = menuElement.querySelector('.' + activeClass);
        const index = all.indexOf(prev || menuElement.querySelector('.' + selectedClass));
        const isTextInput = textInputAllowed(document.activeElement);
        switch (e.data) {
            case 'downArrow':
                return moveNext(prev, all[index + 1] || (options.wrapAround && all[0])) || all[0];
            case 'upArrow':
                return moveNext(prev, all[index - 1] || (options.wrapAround && all[all.length - 1])) || all[0];
            case 'space':
                if (isTextInput) {
                    return;
                }
        ...
        if ((!e.data || e.data.length === 1) && showKeyboardOnFocus && !isTextInput && !currentInput) {
            createDelegatedInput(menuElement, function (text) {
                const prev = menuElement.querySelector('.' + activeClass);
                const index = all.indexOf(prev) + (text.length > 1 ? 0 : 1);
                const predicate = (v) => {
    ...
    return dom.on(container, {
        ...
        focusout: function () {
            menuElement = null;
            if (currentMenu && currentMenu.container === container) {
                currentMenu = null;
            }
        }
    });
...
export class MenuKeystrokeMixin extends StatefulMixin {
    ...
    select() {
        return !!any(this.elements(), function (element) {
            if (currentMenu && currentMenu.container === element) {
                return currentMenu.handleEvent({ data: 'enter' });
            }
        });
    }
}
